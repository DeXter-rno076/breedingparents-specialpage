...
class JSONSubtree extends VisualSubtree {
    protected function instantiateVisualSubtree (VisualPreparationSubtree $subtree): JSONSubtree {
        return new JSONSubtree($subtree);
    }
    protected function createNodeIcon (VisualPreparationNode $node): JSONLink {
        $icon = new JSONImg($node);
        return new JSONLink($node, $icon);
    }
    protected function instantiateVisualLine (int $x1, int $y1, int $x2, int $y2, int $groupId): JSONLine {
        return new JSONLine($x1, $y1, $x2, $y2, $groupId);
    }
    protected function instantiateVisualConnection (VisualLine $line,
            int $groupId, string $text = null): JSONConnection {
        return new JSONConnection($line, $groupId, $text);
    }
    protected function instantiateVisualCircle (int $x, int $y,
            int $r, string $color, VisualPreparationNode $node): JSONCircle {
